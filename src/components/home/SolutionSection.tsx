...
const SolutionSection = () => {
  return (
    <section id="solutions" className="scroll-mt-28 bg-[#020C11] py-[72px] sm:py-24">
      <div className="mx-auto max-w-[1200px] px-5 sm:px-6 lg:px-10">
        <div className="grid gap-14 lg:grid-cols-[360px_1fr] lg:gap-20">
          <div>
            <p className="mb-4 text-xs font-semibold uppercase tracking-normal text-[#71B7FF]">
              Solutions
            </p>
            <h2 className="text-[34px] font-medium leading-tight tracking-normal text-white sm:text-5xl">
              All of our solutions are tailor-made to your needs
            </h2>

            <button className="mt-9 rounded-full border border-white/20 px-8 py-4 text-sm font-semibold uppercase tracking-normal text-white transition hover:border-white/40 hover:bg-white/10">
              REQUEST DEMO
            </button>
          </div>
          ...
          <div className="grid gap-6 md:grid-cols-2">
            {solutions.map((item, index) => (
              <article
                key={item.title}
                className="rounded-3xl border border-white/10 bg-white/[0.03] p-7 transition duration-300 hover:-translate-y-1 hover:border-[#0878FF]/45 hover:bg-white/[0.05]"
              >
                ...
                <button className="mt-6 text-sm font-semibold uppercase tracking-normal text-[#138BFF]">
                  LEARN MORE -&gt;
                </button>
              </article>
            ))}
